...
save_root = opt.checkpoints_dir + '/output'
...
for i, data_i in enumerate(dataloader):
    ...
    if opt.save_per_img:
        ...
        if not os.path.exists(save_root + '/test/' + opt.name + '/tmp'):
            os.makedirs(save_root + '/test/' + opt.name + '/tmp')

        if opt.dataset_mode == 'deepfashion':
            label = data_i['label'][:, :3, :, :]
        elif opt.dataset_mode == 'celebahqedge':
            # print (data_i['label'].shape)
            label = data_i['label'][:, 10:11, :, :].expand(-1, 3, -1, -1).float()
        else:
        ...
        try:
            imgs = (imgs + 1) / 2
            vutils.save_image(imgs, save_root + '/test/' + opt.name + '/tmp/' + str(i) + '.png',
