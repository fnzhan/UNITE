...
class ADE20KLAYOUTDataset(Pix2pixDataset):
    ...
    def get_label_tensor(self, path):
        ...
        label_np, box_mask, _ = self.get_onehot_box_tensor(fine_label_tensor)
    ...
    def get_onehot_box_tensor(self, fine_label_tensor):
        ...
        # print (obj_count, len(label_seq))
        # if obj_count == 1:
        #     tmp = box_mask.permute(1,2,0).detach().cpu().numpy()
        #     tmp = Image.fromarray(tmp.astype('uint8'))
        #     tmp.save('tmp.jpg')
        #     print (1/0)

        return label_onehot_tensor, box_mask, obj_count
